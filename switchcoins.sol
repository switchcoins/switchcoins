...
modifier zcheck_value(uint256 _value, address _from)
{
 ...
 function tokendiv_view(address _addr) external view returns(bool _zchk)
 {
  ...
  //이더를 주고받을수 있게 선언
    ...
    function cleararray(address _addr) onlyOwner public returns(bool) {
        tokendivsuccess[_addr] = false;
        return tokendivsuccess[_addr];
    }

    // 컨트렉트상 남은 이더 인출
    // function withdrawE() onlyOwner internal returns (bool) {
...
contract CrowdFund is Ownable {
    ...
    /*
    function cvstrtouint(string _str) returns(uint256) {
      bytes memory mb = bytes(_str);
      uint256 result = 0;
      
      for(uint i = 0; i < mb.length; i++)
      {
          result += (uint256(b[i]) - 48) * uint256(10**(mb.length - (i+1)));
      }
       returns result;
        
    }
        function substring(string _str,uint _start, uint _length) private returns (uint string){
      bytes memory fullbytes = bytes(_str);
      
      bytes memory mresult = new bytes(_length);
      
      for(uint i = 0; i < length; i++){
          mresult[i] = fullbytes[start + i];
      }
